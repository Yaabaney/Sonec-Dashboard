...
function applyKpiCardStyles(kpis) {
  ...
  else if (losses > 12) lossesCard.classList.add("warn");
...
function renderGenerationMixChart(kpis) {
  ...
  const generated = Number(kpis.estimated_losses_mwh || 0);
  const totalGeneratedApprox = (Number(kpis.solar_output_mwh || 0) + Number(kpis.battery_output_mwh || 0)) > 0
    ? null
    : null;
...
function getUtilityStatus(item) {
  ...
  if (item.reserve_margin_percent < 10 || item.losses_percent > 12 || item.unplanned_outages >= 2) {
    return "Amber";
  }
  return "Green";
}
...
function buildUtilityData(kpis) {
  ...
  const utilities = baseUtilities.map((u, index) => {
    ...
    const losses = index === 0 ? kpis.losses_percent + 0.5 : index === 1 ? kpis.losses_percent + 1.2 : kpis.losses_percent - 0.8;
...
function renderDashboard(dashboardData, alertsData, summaryData) {
  ...
  setText("executive-summary", shortenText(summaryData.executive_summary, 210));
  setText("operations-summary", shortenText(summaryData.operations_summary, 210));
...
somaliaMapInstance.fitBounds(somaliaBounds);
  ...
  // Main marker: Mogadishu
  L.marker([2.0469, 45.3182])
    .addTo(somaliaMapInstance)
    .bindPopup(`
      <div class="map-popup-title">Mogadishu</div>
      <div class="map-popup-text">Primary operating and reporting centre</div>
    `);

  // Example utility points
  L.marker([10.3182, 43.2864])
    .addTo(somaliaMapInstance)
    .bindPopup(`
      <div class="map-popup-title">Hargeisa</div>
      <div class="map-popup-text">Potential northern utility location</div>
    `);

  L.marker([8.4064, 48.4828])
    .addTo(somaliaMapInstance)
    .bindPopup(`
      <div class="map-popup-title">Bosaso</div>
      <div class="map-popup-text">Potential network and logistics hub</div>
    `);

  L.marker([0.3556, 42.5453])
    .addTo(somaliaMapInstance)
    .bindPopup(`
      <div class="map-popup-title">Kismayo</div>
      <div class="map-popup-text">Potential southern utility location</div>
    `);
}
